<!-- Footer -->
<footer class="bg-primary mt-[300px]">
    <div class="mx-auto w-full max-w-[1280px]">
        <div class="relative p-4 py-6 lg:py-8 lg:pt-[224px]">
            <div class="absolute left-0 -top-40 w-full px-8 lg:px-16 lg:py-8 bg-primaryDark rounded-3xl h-[320px]">
                <div class="relative flex h-full items-center">
                    <div class="text-white flex flex-col max-w-4xl">
                        <h1 class="text-4xl font-bold">Subscribe to Get More Benefits</h1>
                        <p class="text-xl mt-6">Contrary to popular belief, Lorem Ipsum is not simply random text. It has
                            roots in a piece of classical Latin.
                        </p>
                        <div class="flex items-center mt-12">
                            <input type="email" placeholder="youremail123@gmail.com"
                                class="h-14 w-max lg:w-[440px] p-4 focus:outline-none text-gray-700 border-none" />
                            <button class="h-14 w-max lg:w-[188px] bg-primary text-white font-semibold px-4 py-2">
                                SUBSCRIBE
                            </button>
                        </div>
                    </div>
                    <div class="absolute hidden lg:block bottom-0 right-0">
                        <img src="{{ asset('assets/images/avatar-2.png') }}" alt="" class="w-[378px] h-[418px]">
                    </div>
                </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-12">
                <!-- Logo and Description -->
                <div class="mb-6 md:mb-0">
                    <div class="flex items-center gap-2">
                        <img src="{{ asset('assets/icons/healthly-light.png') }}" alt="" class="w-10 h-10">
                        <a href="{{ url('/') }}" class="text-3xl font-semibold mr-4 text-white">Healthly</a>
                    </div>
                    <p class="mt-8 text-xl text-white font-light max-w-xl">Jl. Kyai Moch. Syafei Gg 1 No.2450,
                        RT.04/RW.08,
                        Kebondalem, Purwokerto Lor, Kec. Purwokerto Tim., Kabupaten Banyumas, Jawa Tengah 53114</p>
                    <div class="flex justify-between mt-12 w-[80px] md:w-1/4">
                        <!-- Social Media Icons -->
                        <a href="#"
                            class="group relative text-primary hover:text-primary transition-all duration-200 ease-[cubic-bezier(0.68,-0.55,0.265,1.55)] p-2 rounded-full bg-white">
                            <span
                                class="tooltip absolute top-0 left-1/2 -translate-x-1/2 -translate-y-full bg-white text-primary text-xs py-1 px-2 rounded-md shadow-md opacity-0 group-hover:opacity-100 group-hover:-top-4 transition-all duration-300 ease-[cubic-bezier(0.68,-0.55,0.265,1.55)] pointer-events-none">Facebook</span>
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                <path
                                    d="M17.525,9H14V7c0-1.032,0.084-1.682,1.563-1.682h1.868v-3.18C16.522,2.044,15.608,1.998,14.693,2 C11.98,2,10,3.657,10,6.699V9H7v4l3-0.001V22h4v-9.003l3.066-0.001L17.525,9z">
                                </path>
                            </svg>
                        </a>
                        <a href="#"
                            class="group relative text-primary hover:text-primary transition-all duration-200 ease-[cubic-bezier(0.68,-0.55,0.265,1.55)] p-2 rounded-full bg-white">
                            <span
                                class="tooltip absolute top-0 left-1/2 -translate-x-1/2 -translate-y-full bg-white text-primary text-xs py-1 px-2 rounded-md shadow-md opacity-0 group-hover:opacity-100 group-hover:-top-4 transition-all duration-300 ease-[cubic-bezier(0.68,-0.55,0.265,1.55)] pointer-events-none">Instagram</span>
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                <path fill-rule="evenodd"
                                    d="M12.315 2c2.43 0 2.784.013 3.808.06 1.064.049 1.791.218 2.427.465a4.902 4.902 0 011.772 1.153 4.902 4.902 0 011.153 1.772c.247.636.416 1.363.465 2.427.048 1.067.06 1.407.06 4.123v.08c0 2.643-.012 2.987-.06 4.043-.049 1.064-.218 1.791-.465 2.427a4.902 4.902 0 01-1.153 1.772 4.902 4.902 0 01-1.772 1.153c-.636.247-1.363.416-2.427.465-1.067.048-1.407.06-4.123.06h-.08c-2.643 0-2.987-.012-4.043-.06-1.064-.049-1.791-.218-2.427-.465a4.902 4.902 0 01-1.772-1.153 4.902 4.902 0 01-1.153-1.772c-.247-.636-.416-1.363-.465-2.427-.047-1.024-.06-1.379-.06-3.808v-.63c0-2.43.013-2.784.06-3.808.049-1.064.218-1.791.465-2.427a4.902 4.902 0 011.153-1.772A4.902 4.902 0 015.45 2.525c.636-.247 1.363-.416 2.427-.465C8.901 2.013 9.256 2 11.685 2h.63zm-.081 1.802h-.468c-2.456 0-2.784.011-3.807.058-.975.045-1.504.207-1.857.344-.467.182-.8.398-1.15.748-.35.35-.566.683-.748 1.15-.137.353-.3.882-.344 1.857-.047 1.023-.058 1.351-.058 3.807v.468c0 2.456.011 2.784.058 3.807.045.975.207 1.504.344 1.857.182.466.399.8.748 1.15.35.35.683.566 1.15.748.353.137.882.3 1.857.344 1.054.048 1.37.058 4.041.058h.08c2.597 0 2.917-.01 3.96-.058.976-.045 1.505-.207 1.858-.344.466-.182.8-.398 1.15-.748.35-.35.566-.683.748-1.15.137-.353.3-.882.344-1.857.048-1.055.058-1.37.058-4.041v-.08c0-2.597-.01-2.917-.058-3.96-.045-.976-.207-1.505-.344-1.858a3.097 3.097 0 00-.748-1.15 3.098 3.098 0 00-1.15-.748c-.353-.137-.882-.3-1.857-.344-1.023-.047-1.351-.058-3.807-.058zM12 6.865a5.135 5.135 0 110 10.27 5.135 5.135 0 010-10.27zm0 1.802a3.333 3.333 0 100 6.666 3.333 3.333 0 000-6.666zm5.338-3.205a1.2 1.2 0 110 2.4 1.2 1.2 0 010-2.4z"
                                    clip-rule="evenodd" />
                            </svg>
                        </a>
                        <a href="#"
                            class="group relative text-primary hover:text-primary transition-all duration-200 ease-[cubic-bezier(0.68,-0.55,0.265,1.55)] p-2 rounded-full bg-white">
                            <span
                                class="tooltip absolute top-0 left-1/2 -translate-x-1/2 -translate-y-full bg-white text-primary text-xs py-1 px-2 rounded-md shadow-md opacity-0 group-hover:opacity-100 group-hover:-top-4 transition-all duration-300 ease-[cubic-bezier(0.68,-0.55,0.265,1.55)] pointer-events-none">Twitter</span>
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                <path
                                    d="M8.29 20.251c7.547 0 11.675-6.253 11.675-11.675 0-.178 0-.355-.012-.53A8.348 8.348 0 0022 5.92a8.19 8.19 0 01-2.357.646 4.118 4.118 0 001.804-2.27 8.224 8.224 0 01-2.605.996 4.107 4.107 0 00-6.993 3.743 11.65 11.65 0 01-8.457-4.287 4.106 4.106 0 001.27 5.477A4.072 4.072 0 012.8 9.713v.052a4.105 4.105 0 003.292 4.022 4.095 4.095 0 01-1.853.07 4.108 4.108 0 003.834 2.85A8.233 8.233 0 012 18.407a11.616 11.616 0 006.29 1.84" />
                            </svg>
                        </a>
                    </div>
                </div>

                <!-- Quick Links -->
                <div class="grid grid-cols-2 md:grid-cols-3 gap-8">
                    <div>
                        <h2 class="mb-6 text-2xl font-semibold text-white">Company</h2>
                        <ul class="text-white">
                            <li class="mb-4">
                                <a href="{{ url('/about-us') }}"
                                    class="text-lg font-light transition duration-300 ease-in-out">About
                                    Us</a>
                            </li>
                            <li class="mb-4">
                                <a href="{{ url('https://api.whatsapp.com/') }}" target="_blank"
                                    class="text-lg font-light transition duration-300 ease-in-out">Contact</a>
                            </li>
                            <li>
                                <a href="{{ url('/course') }}"
                                    class="text-lg font-light transition duration-300 ease-in-out">Our
                                    Course</a>
                            </li>
                        </ul>
                    </div>
                    <div>
                        <h2 class="mb-6 text-2xl font-semibold text-white">Information</h2>
                        <ul class="text-white">
                            <li class="mb-4">
                                <a href="{{ url('/blog') }}" target="_blank"
                                    class="text-lg font-light transition duration-300 ease-in-out">Blog</a>
                            </li>
                            <li class="mb-4">
                                <a href="{{ url('/program') }}" target="_blank"
                                    class="text-lg font-light transition duration-300 ease-in-out">Program</a>
                            </li>
                            <li class="">
                                <a href="{{ url('/faq') }}" target="_blank"
                                    class="text-lg font-light transition duration-300 ease-in-out">FAQ</a>
                            </li>
                        </ul>
                    </div>
                    <div>
                        <h2 class="mb-6 text-2xl font-semibold text-white">Teaching</h2>
                        <ul class="text-white">
                            <li class="mb-4">
                                <a href="{{ url('/become-teacher') }}" target="_blank"
                                    class="text-lg font-light transition duration-300 ease-in-out">Become
                                    a
                                    Teacher</a>
                            </li>
                            <li class="mb-4">
                                <a href="{{ url('/rules-requirements') }}" target="_blank"
                                    class="text-lg font-light transition duration-300 ease-in-out">Rules
                                    &
                                    Requirements</a>
                            </li>
                            <li class="">
                                <a href="{{ url('/help-center') }}" target="_blank"
                                    class="text-lg font-light transition duration-300 ease-in-out">Help
                                    Center</a>
                            </li>
                        </ul>
                    </div>
                </div>
            </div>

            {{-- <hr class="my-6 border-white sm:mx-auto lg:my-8" /> --}}

            <!-- Bottom Section -->
            <div class="flex flex-col md:flex-row items-center justify-center md:justify-between mt-8">
                <span class="text-sm text-white text-center">© 2024 <a href="{{ url('/') }}"
                        class="hover:underline">Healthly™</a>. All Rights Reserved.</span>
                <div class="flex items-center">
                    <a href="{{ url('/terms') }}" target="_blank"
                        class="text-sm text-white text-center hover:underline">Terms</a>
                    <span class="mx-2 text-white text-center">•</span>
                    <a href="{{ url('/privacy') }}" target="_blank"
                        class="text-sm text-white text-center hover:underline">Privacy</a>
                </div>
            </div>
        </div>
    </div>
</footer>
